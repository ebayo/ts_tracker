...
# PLOTS for all images, including without label file (account for background images)
...
def get_label_paths(data_base):
    ...
    if isinstance(data_base['train'], list):
        paths += data_base['train']
    elif data_base['train'] is not None:
        paths.append(data_base['train'])
    if isinstance(data_base['val'], list):
        paths += data_base['val']
    elif data_base['val'] is not None:
        paths.append(data_base['val'])

    return [f.replace('images', 'labels') for f in paths]
...
if __name__ == '__main__':
    ...
    for lp in label_paths:
        ...
        labels += lb
        label_count += lb_c
        bg_img = len(os.listdir(lp.replace('labels', 'images'))) - len(os.listdir(lp))
        label_count += [0]*bg_img
